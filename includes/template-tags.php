...
================================================================================================
*/
function amity_entry_posted_on() {
    $author_avatar_size = apply_filters('amity_author_avatar_size', 100);
    ...
    if ( !is_page() && !post_password_required() && (comments_open() || get_comments_number())) {
        echo '<span class="entry-comments"><b>';
            comments_popup_link( sprintf( __( 'Leave a Comment', 'amity')));
        echo '</b></span>';
    }
}
...
function amity_entry_taxonomies() {
    ...
    if ($cat_list) {
        ...
        __('<i class="fa fa-folder-open-o"></i> Posted In', 'amity'),  
        $cat_list
        );
    }
    ...
    if ($tag_list) {
        ...
        __('<i class="fa fa-tags"></i> Tagged', 'amity'),  
        $tag_list 
        );
    }
}